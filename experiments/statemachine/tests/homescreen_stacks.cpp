...
class Engine : public Leap::Listener {
				...
				virtual void onEnter(const Leap::Frame& frame,
					...
					if (nodeid.compare("swL") == 0) {
						...
						if (e->mSelection >= e->mNumSelections) {
							e->mListPosition[e->mCurrentStack]
								-= e->mNumSelections;
							e->mSelection = 0;
						}
	...
	private:
		...
		std::vector<double> mListPosition;
		...
		void initializeApplication() {
			...
			mNumSelections = 20;
			mListNudge = 0.0;
			for (int i = 0; i < mNumStacks; ++i)
				mListPosition.push_back(0.0);
		...
		void updatePosition() {
			...
			mListPosition[mCurrentStack] +=
				(targetposition - mListPosition[mCurrentStack]) / 20.0f
				- mListNudge * 0.00005f;
		...
		*/
		void renderFrame() {
			...
			for (int stack = 0; stack < mNumStacks; ++stack) {
				y = 30.0 + stack * 300.0 + mStackPosition * 50.0;

				for (int i = 0; i < mNumSelections; ++i) {

					x = 250.0 * sin(arc * i - (mListPosition[stack] * arc));
					z = 100.0 * cos(arc * i - (mListPosition[stack] * arc));
					color = (z + 100.0) / 200.0;
